(ns pulumi.main
  "Kotodama WASM entrypoint for the Pulumi clean-room actor (L4) — Clojure port.
  ...
  facts (`pulumi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pulumi")
(def tier "L4")
...
(def entity-specs
  [{:entity "Update"       :plural "updates"       :id-prefix "pulumi_upd"
    :fields [:kind :result :message :version :resourceCount :startTime :endTime] :required [:kind :result]
    :coerce {:version :int :resourceCount :int :startTime :int :endTime :int} :refs {}}
   {:entity "Stack"        :plural "stacks"        :id-prefix "pulumi_sta"
    :fields [:stackName :orgName :projectName :lastUpdate :resourceCount] :required [:stackName :orgName]
    :coerce {:lastUpdate :int :resourceCount :int} :refs {}}
   {:entity "Policy"       :plural "policies"      :id-prefix "pulumi_pol"
    :fields [:name :displayName :description :enforcementLevel :severity :message] :required [:name :displayName]
    :coerce {} :refs {}}
   {:entity "Deployment"   :plural "deployments"   :id-prefix "pulumi_dep"
    :fields [:status :projectName :stackName :paused :created :modified] :required [:status]
    :coerce {:paused :bool} :refs {}}
   {:entity "Organization" :plural "organizations" :id-prefix "pulumi_org"
    :fields [:name :role] :required [:name :role]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"pulumi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pulumi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pulumi-compat/ready)
